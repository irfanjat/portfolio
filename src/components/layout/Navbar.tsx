  ...
  return (
    ...
    >
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <ul className="hidden items-center gap-1 md:flex">
          {navLinks.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="rounded-lg px-4 py-2 text-sm text-slate-400 transition-colors hover:bg-white/5 hover:text-cyan-300"
              >
                {link.label}
              </a>
            </li>
        ...
        <a
          href="#contact"
          className="hidden rounded-xl border border-cyan-400/30 bg-cyan-500/10 px-4 py-2 text-sm font-medium text-cyan-300 transition hover:glow-cyan md:inline-block"
        >
          Hire Me
        </a>
          ...
          <ul className="flex flex-col gap-1 px-4 py-4">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="block rounded-lg px-4 py-3 text-slate-300"
            ...
            <li>
              <a
                href="#contact"
                className="mt-2 block rounded-xl bg-cyan-500/15 px-4 py-3 text-center text-cyan-300"
                onClick={() => setOpen(false)}
              >
                Hire Me
              </a>
